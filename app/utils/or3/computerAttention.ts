...
function getFindingText(
    ...
    const finding =
        readiness?.findings?.find(
            (item) => item.severity === 'block' || item.severity === 'error',
...
export function getReadinessGuidance(
    readiness: ReadinessResponse | null | undefined,
): AttentionGuidance {
    if (!readiness) {
        return {
            ...
            summary:
                'The app could not reach the readiness check, so it cannot explain the host status with confidence yet.',
    ...
    const detail =
        typeof finding?.fixHint === 'string' && finding.fixHint.trim()
    ...
    if (
        ...
        return {
            ...
            detail:
                detail ??
                'Check the selected workspace and indexing options so the computer tools know where to work.',
            action: {
    ...
    if (
        ...
        return {
            ...
            detail:
                detail ??
                'Finish the provider setup so the assistant can actually reply and use the connected computer.',
            action: {
    ...
    return {
        ...
        detail:
            detail ??
            'Open the health report for the raw checks, then review the recommended settings based on that finding.',
...
export function getBootstrapWarningGuidance(
    ...
    if (!warning) {
        return {
            ...
            summary:
                'The app knows something is off, but it did not receive a structured warning payload.',
