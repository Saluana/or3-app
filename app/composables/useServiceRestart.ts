...
export function useServiceRestart() {
    ...
    async function createTerminalSession(root: FileRoot) {
        return await authSession.retryWithAuth(
            (onAuthChallenge) =>
                api.request<TerminalSessionSnapshot>(
    ...
    async function sendRestartCommand(sessionId: string) {
        await authSession.retryWithAuth(
            (onAuthChallenge) =>
                api.request(
                    `/internal/v1/terminal/sessions/${sessionId}/input`,
                    {
                        method: 'POST',
    ...
    async function restartService() {
        ...
        try {
            ...
            if (!root) {
                throw new Error(
                    'This computer did not expose a writable folder that can reach the or3-intern repo.',
                );
            }
