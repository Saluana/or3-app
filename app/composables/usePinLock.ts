...
function syncReactiveState() {
    ...
    unlockDurationState.value =
        _config?.unlockDurationMs ?? PIN_UNLOCK_DEFAULT_MS;
}
...
function loadLockout() {
    ...
    if (
        (_lockedUntilTs !== null && !Number.isFinite(_lockedUntilTs)) ||
        !Number.isFinite(_failedAttempts)
    ) {
        localStorage.removeItem(PIN_LOCKOUT_KEY);
        return;
    }
    ...
    try {
        ...
        if (parsed.enabled && parsed.salt) {
            return {
                enabled: true,
                salt: parsed.salt,
                unlockDurationMs: sanitizeUnlockDurationMs(
                    parsed.unlockDurationMs,
                ),
            };
        }
        return null;
...
function parsePlainTokenMap(
    raw: string | null,
): Record<string, HostTokenRecord> | null {
    if (!raw) return null;
    try {
        const parsed = JSON.parse(raw) as
            | Record<string, HostTokenRecord>
            | EncryptedTokenBlob;
        if (
            !parsed ||
            typeof parsed !== 'object' ||
            isEncryptedTokenBlob(parsed)
        ) {
            return null;
        }
...
export async function unlock(
    ...
    ensureInit();
    if (!isPinEnabled())
        return { success: false, error: 'PIN lock is not enabled.' };
            ...
            return {
                ...
                error:
                    getAttemptsRemaining() > 0
                        ? `Incorrect PIN. ${getAttemptsRemaining()} attempts remaining.`
...
export async function enable(
    pin: string,
    unlockDurationMs = PIN_UNLOCK_DEFAULT_MS,
): Promise<{ success: boolean; error?: string }> {
    ensureInit();
    if (isPinEnabled())
        return { success: false, error: 'PIN lock is already enabled.' };
    ...
    try {
        ...
        if (tokens === null) {
            ...
            return {
                ...
                error:
                    getAttemptsRemaining() > 0
                        ? `Current PIN is incorrect. ${getAttemptsRemaining()} attempts remaining.`
                        : 'Current PIN is incorrect. Locked out.',
            };
        }
