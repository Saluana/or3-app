...
function refKey(section: string, field: string, channel?: string): string {
    return channel
        ? `${section}::${channel}::${field}`
        : `${section}::${field}`;
}

const FIELD_ALIASES: Record<string, BackendFieldRef> = {
    [refKey('provider', 'kind')]: {
        section: 'provider',
        field: 'provider_preset',
    },
    [refKey('provider', 'apiBase')]: {
        section: 'provider',
        field: 'provider_api_base',
    },
    [refKey('provider', 'apiKey')]: {
        section: 'provider',
    ...
    [refKey('provider', 'embedDimensions')]: {
        section: 'provider',
        field: 'provider_embed_dimensions',
    },
    ...
    [refKey('provider', 'timeoutSeconds')]: {
        section: 'provider',
        field: 'provider_timeout',
    },
    [refKey('provider', 'openaiApiKey')]: {
        section: 'provider',
        field: 'provider_openai_api_key',
    },
    [refKey('provider', 'openrouterApiKey')]: {
        section: 'provider',
        field: 'provider_openrouter_api_key',
    },
    [refKey('provider', 'customApiBase')]: {
        section: 'provider',
        field: 'provider_custom_api_base',
    },
    [refKey('provider', 'customApiKey')]: {
        section: 'provider',
    ...
    [refKey('routing', 'summarizationModel')]: {
        section: 'provider',
        field: 'routing_summarization_model',
    },
    ...
    [refKey('routing', 'embeddingsProvider')]: {
        section: 'provider',
        field: 'routing_embeddings_provider',
    },
    [refKey('routing', 'embeddingsModel')]: {
        section: 'provider',
        field: 'routing_embeddings_model',
    },
    [refKey('routing', 'embeddingsFallbacks')]: {
        section: 'provider',
        field: 'routing_embeddings_fallbacks',
    },
    [refKey('routing', 'embeddingsDimensions')]: {
        section: 'provider',
        field: 'routing_embeddings_dimensions',
    },
    [refKey('favorites', 'openai')]: {
        section: 'provider',
        field: 'favorites_openai',
    },
    [refKey('favorites', 'openrouter')]: {
        section: 'provider',
        field: 'favorites_openrouter',
    },

    [refKey('context', 'historyMaxMessages')]: {
        section: 'runtime',
        field: 'runtime_history_max',
    },
    [refKey('context', 'memoryRetrieveLimit')]: {
        section: 'runtime',
        field: 'runtime_memory_retrieve',
    },
    [refKey('context', 'maxInputTokens')]: {
        section: 'context',
        field: 'context_max_input_tokens',
    },
    [refKey('context', 'outputReserveTokens')]: {
        section: 'context',
        field: 'context_output_reserve',
    },
    [refKey('context', 'safetyMarginTokens')]: {
        section: 'context',
        field: 'context_safety_margin',
    },
    [refKey('memory', 'vectorSearchK')]: {
        section: 'runtime',
        field: 'runtime_vector_k',
    },
    [refKey('memory', 'ftsSearchK')]: {
        section: 'runtime',
        field: 'runtime_fts_k',
    },
    [refKey('memory', 'vectorScanLimit')]: {
        section: 'runtime',
        field: 'runtime_vector_scan_limit',
    },
    [refKey('memory', 'consolidationEnabled')]: {
        section: 'runtime',
        field: 'runtime_consolidation_enabled',
    },
    [refKey('memory', 'consolidationCharLimit')]: {
        section: 'runtime',
        field: 'runtime_consolidation_max_input_chars',
    },

    [refKey('workspace', 'workspaceDir')]: {
        section: 'workspace',
        field: 'workspace_dir',
    },
    ...
    [refKey('cron', 'storePath')]: {
        section: 'automation',
        field: 'automation_cron_store_path',
    },
    [refKey('heartbeat', 'enabled')]: {
        section: 'automation',
        field: 'automation_heartbeat_enabled',
    },
    [refKey('heartbeat', 'intervalSeconds')]: {
        section: 'automation',
        field: 'automation_heartbeat_interval',
    },
    [refKey('triggers', 'fileWatch.enabled')]: {
        section: 'automation',
        field: 'automation_filewatch_enabled',
    },
    [refKey('triggers', 'fileWatch.debounceMs')]: {
        section: 'automation',
        field: 'automation_filewatch_debounce',
    },
    [refKey('triggers', 'fileWatch.pollIntervalMs')]: {
        section: 'automation',
        field: 'automation_filewatch_poll_seconds',
    },
    [refKey('triggers', 'webhook.enabled')]: {
        section: 'automation',
        field: 'automation_webhook_enabled',
    },
    [refKey('triggers', 'webhook.bind')]: {
        section: 'automation',
        field: 'automation_webhook_addr',
    },
    [refKey('triggers', 'webhook.secret')]: {
        section: 'automation',
        field: 'automation_webhook_secret',
    },

    [refKey('runtimeProfile', 'value')]: {
        section: 'runtime',
        field: 'runtime_profile',
    },
    [refKey('skills', 'enableExec')]: {
        section: 'skills',
    ...
    [refKey('security', 'audit.keyFile')]: {
        section: 'security',
        field: 'security_audit_key_file',
    },
    [refKey('security', 'secretStore.enabled')]: {
        section: 'security',
        field: 'security_secret_store_enabled',
    },
...
function resolveFieldRef(
    ...
    return (
        FIELD_ALIASES[refKey(section, field, channel)] ?? {
            section,
            field,
            channel,
        }
...
function findField(
    ...
    return fieldCache.value.find(
        (entry) =>
            refKey(entry.section, entry.field.key, entry.channel) === key,
    )?.field;
}

function availableFieldRef(
    control: SimpleSettingControl,
): BackendFieldRef | undefined {
...
function buildValueIndex(): Record<string, unknown> {
    ...
    for (const entry of fieldCache.value) {
        ...
        for (const [aliasKey, target] of Object.entries(FIELD_ALIASES)) {
            if (
                refKey(entry.section, entry.field.key, entry.channel) !==
                refKey(target.section, target.field, target.channel)
            )
                continue;
            const [section, maybeChannelOrField, maybeField] =
                aliasKey.split('::');
...
function buildOverrideMap(
    ...
    for (const change of changes) {
        ...
        map.set(
            refKey(resolved.section, resolved.field, resolved.channel),
            change.value,
        );
    }
    return map;
...
function isHostUnreachableError(err: unknown): boolean {
    const maybeError = err as { code?: unknown; message?: unknown } | null;
    return (
        maybeError?.code === 'host_unreachable' ||
        maybeError?.message === 'Could not reach the selected computer.'
    );
}
...
export function useSimpleSettings() {
    ...
    async function ensureLoaded(targetSection?: SimpleSettingSectionKey) {
        ...
        for (const s of sections) {
            for (const c of s.controls) {
                for (const ref of c.fieldRefs) {
                    ...
                    pairs.add(
                        resolved.channel
                            ? `${resolved.section}::${resolved.channel}`
                            : resolved.section,
                    );
                }
            }
        ...
        for (const key of pairs) {
            if (loadedSections.value.has(key) || loadingSections.value.has(key))
                continue;
            ...
            try {
                ...
                if (result?.fields) {
                    for (const f of result.fields) {
                        ...
                        const idx = fieldCache.value.findIndex(
                            (e) =>
                                refKey(e.section, e.field.key, e.channel) === k,
                        );
                ...
                if (isHostUnreachableError(err)) return;
                throw err;
            } finally {
    ...
    ): SimpleSettingSection | undefined {
