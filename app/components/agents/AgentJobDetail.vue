      ...
      <div v-if="job" class="flex h-full flex-col">
        ...
        <div class="flex-1 overflow-y-auto px-5 py-5">
          <div class="space-y-6">
              ...
              >
                <li v-for="entry in timeline" :key="entry.id" class="relative">
                  ...
                  >
                    <Icon
                      :name="entry.icon"
                      :class="['size-3', entry.spin && 'animate-spin']"
                    />
                  </span>
                  <p
                  ...
                  <p class="text-[11px] text-(--or3-text-muted)">
                    {{ entry.time }}
                  </p>
                </li>
              </ol>
            ...
            <section v-if="showResultSection">
              <div class="flex items-center justify-between gap-2">
                ...
                >
                  RESULT
                  <span
                    v-if="hasArtifact && !fullResultAvailable"
                ...
                <div class="flex items-center gap-1">
                  ...
                  >
                    <Icon
                      ...
                      "
                      :class="['size-3', fullResultLoading && 'animate-spin']"
                    />
                    {{ fullResultLoading ? "Loading…" : "View full result" }}
                  </button>
                  <button
                    v-else-if="fullResultAvailable"
                    type="button"
                  ...
                  >
                    <Icon name="i-pixelarticons-arrow-up" class="size-3" />
                    Show preview
                  </button>
                  <button
                    type="button"
              ...
              >
                <div
                  :class="[
                    ...
                    fullResultAvailable ? 'max-h-112' : 'max-h-80',
                  ]"
                >
                  <StreamingMarkdown
            ...
            >
              The task finished but didn't return a text preview. You can retry
              or open it in chat to dig deeper.
            </section>
            ...
            <section v-if="toolCalls.length">
                ...
                >
                  ...
                  >
                    <div class="flex min-w-0 items-center gap-2">
                      ...
                      >
                        {{ formatToolTime(call.finishedAt) }}
                      </span>
                    </div>
                    <Icon
                      :name="
                  ...
                  >
                    ...
                    <p
                      v-if="!call.arguments && !call.result && !call.error"
              ...
              >
                Live event log expired — only persisted summary is shown.
              </p>
            </section>
            <section
              v-else-if="
                isTerminal && !liveLoadFailed && !toolCalls.length && !isCli
              "
            ...
            <section v-if="isCli && hasCliOutput">
              ...
              <div
                ...
                :class="{
                  'border-(--or3-danger)/60': cliOutputFilter === 'stderr',
                  'border-(--or3-green)': cliOutputFilter === 'stdout',
                }"
              >
                <pre
                  class="max-h-96 overflow-y-auto px-4 py-3 font-mono text-[12px] leading-5 whitespace-pre-wrap"
                  :class="{
                    'text-(--or3-danger)': cliOutputFilter === 'stderr',
                    'text-(--or3-text)': cliOutputFilter !== 'stderr',
                  }"
                  >{{ cliOutputText }}</pre
                >
              </div>
            ...
            <section v-if="isCli && hasCliStructuredEvents" class="space-y-2">
              <button
                type="button"
              ...
              >
                <Icon
                  ...
                  class="size-3"
                />
                {{ cliRawExpanded ? "Hide" : "Show" }} raw events ({{
                  cliStructuredEventCount
                }})
              </button>
              <div
                v-if="cliRawExpanded"
                ...
                >
                  ...
                  <dd class="flex min-w-0 items-center gap-1.5 text-right">
                    ...
                    <button
                      ...
                      @click="copyDetail(row.id, row.value)"
                    >
                      <Icon
                        :name="
                          copiedRow === row.id
        ...
        >
          <div class="flex flex-col gap-2">
            ...
            >
              <Icon name="i-pixelarticons-message" class="size-4" />
              Continue in chat
            </button>
            <button
            ...
            >
              ...
              >
                <Icon
                  :name="
                    busy ? 'i-pixelarticons-loader' : 'i-pixelarticons-close'
                  "
              ...
              >
                <Icon
                  :name="
                    busy ? 'i-pixelarticons-loader' : 'i-pixelarticons-redo'
                  "
                  :class="['size-4', busy && 'animate-spin']"
                />
                Try again
...
import { isActiveStatus, isCliJob, isTerminalStatus } from "~/utils/or3/jobs";
import {
...
async function loadJobDetail(jobId: string) {
  ...
  try {
    ...
    liveEvents.value = Array.isArray(snapshot.events) ? snapshot.events : [];
...
const toolCalls = computed<ToolCallEntry[]>(() => {
  ...
  for (const event of liveEvents.value) {
    if (event.type === "tool_call") {
      ...
      const target = matchIdx !== undefined ? entries[matchIdx] : undefined;
      const resultText =
...
const hasCliStructuredEvents = computed(
  () =>
    !!(
      (props.job?.structured_events && props.job.structured_events.length) ||
