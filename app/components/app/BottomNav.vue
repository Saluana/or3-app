<template>
    <nav
        class="fixed inset-x-0 bottom-0 z-50 border-t border-(--or3-border) bg-[#FFFCF5]/95 px-3 pt-2 backdrop-blur or3-bottom-nav-safe or3-bottom-nav"
        ...
        <div class="mx-auto grid max-w-xl grid-cols-5 items-end gap-1">
            <NuxtLink
                ...
                :class="
                    isActive(item.to)
                        ? 'text-(--or3-green)'
                        : 'text-(--or3-text-muted) hover:text-(--or3-text)'
                "
            >
                <span
                    v-if="item.center"
                ...
                <span :class="item.center ? 'sr-only' : 'font-medium'">{{
                    item.label
                }}</span>
                <span
