    ...
    >
        <div class="flex items-start justify-between gap-3">
            <div class="min-w-0 flex-1">
                ...
                <p class="mt-1 text-xs leading-5 text-(--or3-text-muted)">{{ control.description }}</p>
            </div>
            <span
        ...
        >
            ...
            Recommended: {{ control.recommended.label ?? String(control.recommended.value) }}
        </p>
        ...
        >
            <p class="mb-2 font-mono text-[11px] uppercase tracking-wide text-(--or3-text-muted)">Custom values</p>
            <div class="space-y-2">
                <div
                ...
                >
                    <code class="font-mono text-xs text-(--or3-text-muted)">{{ ref.section }}.{{ ref.field }}</code>
                    <UInput
                        :model-value="String(rawValueAt(ref) ?? '')"
                        size="xs"
                        class="w-32"
                        @update:model-value="(v: string) => onChange({ section: ref.section, field: ref.field, channel: ref.channel, value: coerceNumberLike(v) })"
                    />
                </div>
            </div>
        </div>
...
import type { SimpleSettingChange, SimpleSettingControl, SimpleSettingPreset } from '~/settings/simpleSettings'
...
const activePreset = computed<SimpleSettingPreset | null>(() => simple.detectPreset(props.control))
const activePresetId = computed<string | null>(() => activePreset.value?.id ?? null)

function rawValueAt(ref: { section: string; field: string; channel?: string }): unknown {
...
const selectedChoice = computed(() => choiceItems.value.find((c) => c.value === String(currentValue.value ?? '')))
...
function onToggle(value: boolean) {
    ...
    emit('change', [value ? props.control.toggle.on : props.control.toggle.off])
}
...
function onChoice(value: { value: string } | string | null) {
    ...
    emit('change', [{ section: ref.section, field: ref.field, channel: ref.channel, value: v }])
}
