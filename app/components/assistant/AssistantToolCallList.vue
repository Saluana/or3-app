    ...
    >
      <summary class="flex cursor-pointer items-start gap-2 px-3 py-2.5 text-sm text-(--or3-text)">
        <span class="mt-0.5 inline-flex size-5 shrink-0 items-center justify-center rounded-full"
          :class="call.status === 'running' ? 'bg-(--or3-green-soft) text-(--or3-green-dark)' : call.status === 'error' ? 'bg-(--or3-danger-soft) text-(--or3-danger)' : 'bg-(--or3-surface) text-(--or3-green)'">
      ...
      <div class="space-y-2 border-t border-(--or3-border) px-3 py-2.5 text-xs leading-5 text-(--or3-text-muted)">
        ...
        <div v-if="call.error">
          <p class="mb-1 font-mono uppercase tracking-[0.14em] text-(--or3-danger)">Error</p>
          <pre class="overflow-x-auto rounded-xl bg-(--or3-surface) px-3 py-2 whitespace-pre-wrap text-(--or3-danger)">{{ call.error }}</pre>
        </div>
      </div>
    </details>
  </div>
