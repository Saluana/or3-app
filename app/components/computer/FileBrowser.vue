<template>
    <div class="or3-fb-root">
        <FileToolbar
            ...
            @toggle-favorite="toggleCurrentDirectoryFavorite"
            @open-favorite="openFavoriteDirectory"
        ...
        <DangerCallout
            v-if="fileError"
            tone="caution"
            title="File browser needs attention"
        >
            {{ fileError }}
        </DangerCallout>

        <!-- Pairing required state -->
        <SurfaceCard v-if="!roots.length" class-name="text-center">
            <div class="grid place-items-center gap-3 py-8">
                ...
                <p class="max-w-sm text-sm leading-6 text-(--or3-text-muted)">
                    Pair this computer and allow file access in OR3 to browse
                    its folders here.
                </p>
            </div>
        </SurfaceCard>
        ...
        <template v-else>
            ...
            >
                ...
                >
                    <p
                        class="text-[10px] font-semibold uppercase tracking-[0.18em] text-(--or3-text-muted)"
                    >
                        {{
                            searchMode === 'path'
                                ? 'Path lookup'
                    ...
                    <p class="text-[10px] text-(--or3-text-muted)">
                        {{ searchResults.length }} match<span
                ...
                <div
                    ...
                    class="divide-y divide-(--or3-border) max-h-80 overflow-y-auto"
                >
                    <button
                ...
                <div
                    v-else
                    class="px-4 py-6 text-center text-sm text-(--or3-text-muted)"
                >
                    {{
                        searchMode === 'path'
            ...
            <SurfaceCard class-name="!p-0 overflow-hidden">
                ...
                >
                    <p
                        class="truncate text-[10px] font-semibold uppercase tracking-[0.18em] text-(--or3-text-muted)"
                        :title="entriesPanelTitle"
                    >
                        {{ entriesPanelTitle }}
                    </p>
                    <span
                        class="shrink-0 rounded-full border border-(--or3-border) bg-(--or3-surface-soft) px-2 py-0.5 text-[10px] text-(--or3-text-muted)"
                    >
                ...
                <div v-if="loadingFiles" class="space-y-2 p-3">
                    <div
                        v-for="index in 5"
                        :key="index"
                        class="h-14 rounded-2xl bg-(--or3-surface-soft)"
                    />
                </div>
            ...
            <template #content>
                <div class="space-y-4 p-5">
                    <div>
                        ...
                        >
                            This creates a folder inside
                            {{
                                currentPath === '.'
                                    ? activeRoot?.label || 'the current area'
                                    : currentPath
                            }}.
                        </p>
                    </div>
...
import { useFavoriteDirectories } from '~/composables/useFavoriteDirectories';
...
async function openMemoryTools() {
    ...
    await Promise.allSettled([
        loadAuditStatus().catch(() => undefined),
        loadMemoryShortcuts().catch(() => undefined),
    ]);
}
...
const activeRoot = computed(
    () => roots.value.find((root) => root.id === currentRootId.value) ?? null,
);
const isWritableRoot = computed(() => Boolean(activeRoot.value?.writable));
const currentDirectoryPath = computed(() => currentPath.value?.trim() || '.');
const currentDirectoryFavorite = computed(() =>
    isFavorite(currentRootId.value, currentDirectoryPath.value),
);
...
const detailActions = computed(() => {
    ...
    if (isDir) {
        list.push({
            ...
            onSelect: () => openSelectedDirectory(),
        });
    } else {
        list.push({
            id: 'preview',
            label: 'Open',
            icon: 'i-pixelarticons-eye',
            loading: openingFile.value,
            onSelect: () => openEntryInBrowser(entry),
        });
    ...
    list.push(
        {
            ...
            onSelect: () => copyEntryPath(entry),
        },
        {
            id: 'terminal',
            label: 'Terminal',
            icon: 'i-pixelarticons-terminal',
            onSelect: () => openTerminalAt(entry),
        },
        {
            id: 'ask',
            label: 'Ask AI',
            icon: 'i-pixelarticons-message-text',
            onSelect: () => askAboutEntry(entry),
        },
    );
...
function isTextLikeFile(entry: Pick<FileEntry, 'name' | 'mime_type'>) {
    ...
    if (
        ...
        return true;
    return /\.(md|txt|json|ya?ml|toml|ini|cfg|conf|env|csv|ts|tsx|js|jsx|vue|go|py|rb|php|java|kt|swift|sql|html|css|scss|sh)$/i.test(
        entry.name,
    );
}
...
async function refresh() {
    ...
    const rootId =
        roots.value.find((root) => root.id === currentRootId.value)?.id ||
        roots.value[0]?.id;
    if (!rootId) return;
    const path =
...
async function openFavoriteDirectory(favorite: FavoriteDirectory) {
    const previousRootId = currentRootId.value;
    clearSearch();
    selectedEntry.value = null;
    resetPreview();
    await listDirectory(favorite.rootId, favorite.path).catch(() => undefined);
    await syncRoute(favorite.rootId, favorite.path);
    if (memoryToolsLoaded && favorite.rootId !== previousRootId) {
        await loadMemoryShortcuts().catch(() => undefined);
    }
}
...
async function downloadEntry(entry: FileEntry) {
    ...
    try {
        ...
        toast.add({
            ...
            description: error?.message || 'Could not download that file.',
            color: 'error',
...
function openTerminalAt(entry: FileEntry) {
    const path =
        entry.type === 'directory' ? entry.path : parentPath(entry.path);
    void navigateTo({
        path: '/computer/terminal',
        query: { root: currentRootId.value, path, fresh: '1' },
    });
}
...
function askAboutEntry(entry: FileEntry) {
    void programmaticSend(
        'main',
        `Help me understand this file on my computer: ${entry.path}`,
    );
}
...
function folderSearchMatches(query: string, path: string, name: string) {
    ...
    return query
        ...
        .filter(Boolean)
        .every(
            (token) => lowerName.includes(token) || lowerPath.includes(token),
        );
}
        ...
        .map(
            (entry) =>
                ({
                    ...
                    root_label: activeRoot.value?.label || '',
                    name: entry.name,
                    path: entry.path,
                    type: entry.type,
...
async function runSearch() {
    ...
    const local = currentDirectorySearchMatches(query);
    const remote = await searchWorkspaceFiles(
        query,
        50,
        currentRootId.value,
    ).catch(() => [] as FileSearchItem[]);
    ...
    const prefix = currentPath.value.endsWith('/')
        ...
        : `${currentPath.value}/`;
    const merged = new Map<string, FileSearchItem>();
    for (const item of local) merged.set(`${item.root_id}:${item.path}`, item);
    for (const item of remote) {
        if (item.path === currentPath.value || item.path.startsWith(prefix)) {
            merged.set(`${item.root_id}:${item.path}`, item);
        }
    }
    searchResults.value = [...merged.values()];
...
async function loadMemoryShortcuts() {
    ...
    const candidates = [
        {
            ...
            description: "OR3's durable note.",
            path: 'MEMORY.md',
        },
        {
            label: 'Today note',
            description: "Today's daily memory log.",
            path: `memory/${todayStamp(0)}.md`,
        },
        {
            label: 'Yesterday note',
            description: "Yesterday's daily memory log.",
            path: `memory/${todayStamp(-1)}.md`,
        },
        {
            label: 'Memory folder',
            description: 'All daily notes and support files.',
            path: 'memory',
        },
        {
            label: 'Identity note',
            description: 'Assistant identity and behavior guide.',
            path: 'SOUL.md',
        },
        {
            label: 'User note',
            description: 'Human context note read at startup.',
            path: 'USER.md',
        },
    ] as const;

    const resolved = await Promise.all(
        candidates.map(async (candidate): Promise<MemoryShortcut | null> => {
    ...
    memoryShortcuts.value = resolved.filter(
        (item): item is MemoryShortcut => item !== null,
    );
}
...
async function handleRebuild(target: 'memory' | 'docs') {
    try {
        ...
        toast.add({
            ...
            description:
                target === 'memory'
                    ? 'The memory index was rebuilt for saved notes.'
                    : 'The document index was rebuilt for approved document roots.',
            color: 'success',
...
async function handleVerifyAudit() {
    try {
        ...
        toast.add({
            ...
            description: 'The audit chain passed its integrity check.',
            color: 'success',
...
async function confirmCreateFolder() {
    ...
    try {
        ...
        createFolderError.value =
            error?.message ?? 'Could not create that folder.';
    } finally {
...
async function loadPreview(entry: FileEntry) {
    ...
    if ((entry.size || 0) > 512 * 1024 && !isImageLikeFile(entry)) {
        ...
        preview.message =
            'This file is larger than the inline preview limit. Download or open it in your browser instead.';
        return;
    }
    ...
    try {
        ...
        preview.kind = 'unavailable';
        preview.message =
            'Preview is not shown for this file type yet. Use Download or Open in browser.';
    } catch (error: any) {
        preview.kind = 'unavailable';
        preview.message =
            error?.message ?? 'Could not load a preview for this file.';
    } finally {
        preview.loading = false;
...
onMounted(async () => {
    ...
    const rootFromQuery =
        typeof route.query.root === 'string' ? route.query.root : '';
    const pathFromQuery =
        typeof route.query.path === 'string' ? route.query.path : '';
    if (
        rootFromQuery &&
        roots.value.some((root) => root.id === rootFromQuery)
    ) {
        await listDirectory(rootFromQuery, pathFromQuery || '.').catch(
            () => undefined,
        );
