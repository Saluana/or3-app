        ...
        <div class="or3-terminal-page">
            ...
            <div class="or3-terminal-page__bar">
                ...
                <div class="or3-terminal-page__crumbs">
                    <span
                        class="or3-terminal-page__crumb-icon"
                        aria-hidden="true"
                    >
                        <Icon
                    ...
                    <div class="or3-terminal-page__crumbs-text">
                        <p class="or3-terminal-page__title">
                            {{ headlineTitle }}
                        </p>
                ...
                <div class="or3-terminal-page__bar-actions">
                    ...
                    >
                        <span
                            class="or3-terminal-page__pill-dot"
                            :class="{
                                'is-pulsing': session.status === 'running',
                            }"
                        />
                        <span class="or3-terminal-page__pill-label">{{
                            statusLabel
                        }}</span>
                    </span>
                    ...
                    >
                        ...
                        >
                            <Icon
                                name="i-pixelarticons-more-vertical"
                                class="size-4"
                            />
                        </button>
                    </UDropdownMenu>
            ...
            <div
                v-if="bannerMessage"
                class="or3-terminal-page__banner"
                :data-tone="bannerTone"
                role="status"
            >
                <Icon :name="bannerIcon" class="size-4 shrink-0" />
                <p class="or3-terminal-page__banner-copy">
                    {{ bannerMessage }}
                </p>
                <UButton
                    ...
                    <div class="or3-terminal-page__setup">
                        <div class="or3-terminal-page__setup-card">
                            <div class="or3-terminal-page__setup-head">
                                ...
                                <div>
                                    ...
                                    <p class="or3-terminal-page__setup-sub">
                                        Real PTY on your computer — vim, htop,
                                        less all work.
                                    </p>
                                </div>
                            </div>
                            ...
                            <div
                                ...
                                class="or3-terminal-page__resume"
                            >
                                <div class="or3-terminal-page__resume-head">
                                    <p class="or3-terminal-page__resume-title">
                                        Resume active session
                                    </p>
                                    <span
                                        class="or3-terminal-page__resume-count"
                                ...
                                <div class="or3-terminal-page__resume-list">
                                    ...
                                    >
                                        <span
                                            class="or3-terminal-page__resume-shell"
                                            >{{ item.shell }}</span
                                        >
                                        <span
                                            class="or3-terminal-page__resume-path"
                                            >{{ item.cwd }}</span
                                        >
                                    </button>
                                </div>
                            ...
                            <p
                                v-if="fileError"
                                class="or3-terminal-page__setup-error"
                            >
                                {{ fileError }}
                            </p>
                        </div>
...
const explicitRouteRoot = computed(() =>
    typeof route.query.root === 'string' ? route.query.root : '',
);
const explicitRoutePath = computed(() =>
...
const selectedRootLabel = computed(
    () =>
        roots.value.find((root) => root.id === selectedRootId.value)?.label ??
        'Select an area',
);
...
const bannerIcon = computed(() => {
    ...
    return bannerTone.value === 'amber'
        ? 'i-pixelarticons-alert'
        : 'i-pixelarticons-shield';
});
...
function matchingActiveSession(rootId: string, path: string) {
    return activeSessions.value.find(
        (item) => item.root_id === rootId && (item.path || '.') === path,
    );
}
...
async function launchFromRouteTarget() {
    ...
    if (existing?.session_id) {
        const resumed = await attachExistingSession(existing.session_id).catch(
            (error) => {
...
async function handleResize(rows: number, cols: number) {
    await resize(rows, cols).catch((error) =>
        logTerminalActionError('resize', error),
    );
}
...
onMounted(async () => {
    await refreshRoots();
    await listSessions().catch((error) =>
        logTerminalActionError('list sessions', error),
    );
...
.or3-terminal-page {
    ...
    height: calc(
        100dvh - var(--or3-safe-top) - var(--or3-safe-bottom) - 6.5rem
    );
    gap: 8px;
}
