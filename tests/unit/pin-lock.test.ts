...
describe('usePinLock', () => {
  ...
  it('encrypts legacy plaintext session tokens when enabling the pin lock', async () => {
    globalThis.sessionStorage.setItem('or3-app:v1:secure-host-tokens', JSON.stringify({
      'host-1': {
    ...
    const encrypted = globalThis.localStorage.getItem('or3-app:v1:secure-host-tokens')
    expect(encrypted).toContain('"iv"')
    expect(encrypted).not.toContain('paired-token')
    expect(globalThis.sessionStorage.getItem('or3-app:v1:secure-host-tokens')).toBeNull()
  ...
  it('persists failed attempts across reloads before lockout kicks in', async () => {
    ...
    expect(globalThis.localStorage.getItem('or3-app:v1:pin-lockout')).toContain('"failedAttempts":2')

    const freshPinLock = await import(`../../app/composables/usePinLock?reload=attempts`)
  ...
  it('restores the unlocked session across a reload within the grace window', async () => {
    ...
    await expect(enable('1234', PIN_UNLOCK_DEFAULT_MS)).resolves.toMatchObject({ success: true })

    expect(isUnlocked()).toBe(true)
    const freshPinLock = await import(`../../app/composables/usePinLock?reload=grace-active`)
  ...
  it('requires the pin again after the unlock grace window expires', async () => {
    ...
    await expect(enable('1234', unlockDurationMs)).resolves.toMatchObject({ success: true })

    expect(needsUnlock()).toBe(false)
    vi.advanceTimersByTime(unlockDurationMs + 1)
    const freshPinLock = await import(`../../app/composables/usePinLock?reload=grace-expired`)
